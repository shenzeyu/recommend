...
$(document).ready(function(){ 
	//列表
	$(gId).datagrid({
		url:ctx+'/SysPower_json!listJson.do',
		idField:'id',
		fitColumns:true,
		frozenColumns:[[
		...
		columns:[
			getTableHeadOpt(),
			[
				{field:'name',title:'权限名称',width:130,align:'left'},
				{field:'code',title:'权限编号',width:140,align:'left'},
		...
		opt.push(
			{field:'opt',title:'操作',width:100,align:'center', rowspan:2,
				formatter:function(value,rowData,rowIndex){
					var html = '&nbsp;'
					if('' == todo){
						if('system'!=rowData.code && $(systemUpdates).val()=='1'){
...
function searchData(){
    ...
    if(''==powerName && ''==powerCode){
    	alert('查询条件不能为空!');
    }else{
    	realoadGrid(powerName,powerCode);
    }
}
//确定搜索时重新加载datagrid
function realoadGrid(powerName,powerCode){
	var queryParams = $(gId).datagrid('options').queryParams;
	queryParams={"sysPower.name":powerName,"sysPower.code":powerCode};
