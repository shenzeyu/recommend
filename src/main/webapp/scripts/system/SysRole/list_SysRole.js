...
$(document).ready(function(){ 
	//列表
	$(gId).datagrid({
		url:ctx+'/SysRole_json!listJson.do',
		idField:'id',
		fitColumns:true,
		frozenColumns:[[
		...
		columns:[
			getTableHeadOpt(),
			[
				{field:'code',title:'角色编号',width:80,align:'left'},
				...
				formatter:function(value,rowData,rowIndex){
					...
					if('' == todo && $(systemUpdates).val()=='1'){
						html = '<a href="javascript:void(0)" onclick="editData(\''+rowData.id+'\');">编辑</a>'
							 /*+ '&nbsp;'	
							 + '<a href="javascript:void(0)" onclick="selectPower(\''+rowData.id+'\');">选择权限</a>'*/;						
					}					
					return html;
				}
			}			
...
function searchData(){
    ...
    if(''==roleName && ''==roleCode){
    	...
    	realoadGrid(roleName,roleCode);
    }
}
//确定搜索时重新加载datagrid
function realoadGrid(roleName,roleCode){
	var queryParams = $(gId).datagrid('options').queryParams;
	queryParams={"sysRole.name":roleName,"sysRole.code":roleCode};
...
function selectPower(id){
	var url = ctx+'/SysRolePower!edit.do';
	if(id){
		url += '?sysRole.id='+id;
	}
	//弹出框
