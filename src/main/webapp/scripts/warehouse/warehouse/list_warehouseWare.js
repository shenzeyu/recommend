	...
	function getGridOpt(){
		var opt = {
			...
			columns:[
				getTableHeadOpt(),
				[
					{field:'wareCode',title:'货物编号',width:25,align:'left'},
					{field:'wareName',title:'货物名称',width:25,align:'left'},
					{field:'wareSpecificationVal',title:'货物规格',width:25,align:'left'},
					{field:'warehouseName',title:'仓库名称',width:25,align:'left'},
					{field:'wareCount',title:'货物数量',width:20,align:'left'},
					{field:'averageCostInventory',title:'库存平均成本',width:20,align:'left'},
					{field:'totalCostInventory',title:'库存总成本',width:20,align:'left'},
					{field:'sort',title:'序号',width:15,align:'left'}
				]
			],
			pageSize:20,
			rownumbers:true,
...
//获取url参数
function getUrlOpt(){
	var url = ctx+'/WarehouseWare_json!listJson.do?1=1';
	var warehouseId = $('#warehouseId').val();
	if(warehouseId){
		url += '&warehouseWare.warehouseId='+warehouseId;
	}
	return url;
}
