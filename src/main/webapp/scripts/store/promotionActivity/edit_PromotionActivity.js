...
function addUploadImage(theURLElementId){
   var url = ctx+"/FileUpload!edit.do?appType=promotionActivityPic";
...
function getUrlOpt(){
	...
	var url = '';
	if(id != ''){
		url = ctx+'/PromotionActivityGood_json!listJson.do?promotionActivityGood.promoteId='+id;
	}
	return url;
}	
//重新加载
...
function submitSaveForm(){
	...
	if(checkForm()){
		...
		var promotionActivityGoodArr = $(gId).datagrid('getData').rows;//获取json数据集合
		
		if(promotionActivityGoodArr){
			var htmlAll = '';
			var length = promotionActivityGoodArr.length;
			for(var i=0;i<length;i++){
				var promotionActivityGood = promotionActivityGoodArr[i];
				var goodIdHtml = '<input type="hidden" name="goodIdArr" value="'+promotionActivityGood.goodId+'" />'; 						
				var goodNameHtml = '<input type="hidden" name="goodNameArr" value="'+promotionActivityGood.goodName+'" />'; 							
				var priceMarketHtml = '<input type="hidden" name="priceMarketArr" value="'+promotionActivityGood.priceMarket+'" />'; 						
				var pricePromoteHtml = '<input type="hidden" name="pricePromoteArr" value="'+promotionActivityGood.pricePromote+'" />';
		...
		var options = {
			...
			success : function(returnData){
				if(returnData=='true'){
					alert('保存成功!');
					window.parent.addTab('促销活动',ctx+'/promotionActivity!list.do');
				}else{
					alert('保存失败');
				}
			},
