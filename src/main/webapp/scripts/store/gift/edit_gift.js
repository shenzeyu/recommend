...
function addUploadImage(theURLElementId){
   var url = ctx+"/FileUpload!edit.do?appType=giftPic";
...
function getUrlOpt(){
	...
	var url = '';
	if(id != ''){
		url = ctx+'/GiftGood_json!listJson.do?giftGood.promoteId='+id;
	}
	return url;
}	
//重新加载
...
function submitSaveForm(){
	...
	if(checkForm()){
		...
		var giftGoodArr = $(gId).datagrid('getData').rows;//获取json数据集合
		
		if(giftGoodArr){
			var htmlAll = '';
			var length = giftGoodArr.length;
			for(var i=0;i<length;i++){
				var giftGood = giftGoodArr[i];
				var goodIdHtml = '<input type="hidden" name="goodIdArr" value="'+giftGood.goodId+'" />'; 						
				var goodNameHtml = '<input type="hidden" name="goodNameArr" value="'+giftGood.goodName+'" />'; 							
				var priceMarketHtml = '<input type="hidden" name="priceMarketArr" value="'+giftGood.priceMarket+'" />'; 						
				var pricePromoteHtml = '<input type="hidden" name="pricePromoteArr" value="'+giftGood.pricePromote+'" />';
				htmlAll += '<tr>' + goodIdHtml + goodNameHtml + priceMarketHtml + pricePromoteHtml +'</tr>'; 
			}
		}
		var giftGoodTable = $('#giftGoodTable');//订单资料项数据区
		giftGoodTable.html(htmlAll);
		var options = {
			url : ctx+'/gift!save.do',
			async : false,
			type : 'post',
			success : function(returnData){
				if(returnData=='true'){
					alert('保存成功!');
					window.parent.addTab('促销活动',ctx+'/gift!list.do');
				}else{
					alert('保存失败');
				}
			},
		...
		$('#giftGoodForm').ajaxSubmit(options);
	}
}
//表单验证
function checkForm(){
	var titleVal = $("#giftName").val();
	if(titleVal == ''){
		alert('请输入礼品包名称!');
		$("#giftName").focus();
		return false;
...
var editorView = $.extend($.fn.datagrid.defaults.editors, {
	...
	pricePromote: {
        ...
        init: function(container, options){        
        	var html = '<input type="text" name="giftGood.pricePromote" value="0.00" id="pricePromote" class="inputTextBorder" onchange="common.checkNumber(this);flushMoney()"/>';
