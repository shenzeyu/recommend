...
function getColumnsOpt(){
	var opt = [
		...
		{field:'bankAccountCode',title:'收款账户编号',width:15,align:'left'},
		{field:'bankAccountName',title:'收款账户名称',width:15,align:'left'},
	...
	if(checkForm()){
		...
		var options = {
			...
			success : function(returnData){
				if(returnData=='true'){
					alert('保存成功!');
					window.parent.addTab('收款单',ctx+'/collection!list.do');
				}else{
					alert('保存失败');
				}
			},
		...
		$('#CollectionForm').ajaxSubmit(options);
	}
}
//表单验证
function checkForm(){
	...
	//实收金额
	var outOfPocket = $('#moneyCollect').val();
	if(outOfPocket == ''){
		alert('实收金额不能为空');
		$('#moneyCollect').focus();
		return false;
	...
	if(dataArr && more){
		...
		for(var i=0;i<dataLen;i++){
			...
			if(isExist){
				...
				if(addRowflag){
					rowData = addCollectionRow(more);	//增加行
				}else{
...
function selectCustomer(){
	...
	if(data){
		...
		$('#moneyAccount').val(data.payables);
	}
}
