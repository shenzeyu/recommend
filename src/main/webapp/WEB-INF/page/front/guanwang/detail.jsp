...
<div class="box">
  ...
  <div class="details01_text">
   ...
   <h3>产品编号：${good.code }</h3>
    <h3>市 场 价： <samp class="font">¥</samp>${good.priceMarket}</h3>
	 <h3>本 店 价：<span>¥${good.price}</span>为您节省了：
   ...
   <div class="justify2">
   <samp>：</samp><span id="goodCount"><samp style="margin-left: 10px"><c:if test="${warehousecount>0}">有货</c:if><c:if test="${warehousecount<1}">无货</c:if></samp></span>
   </div>
   </h4>
   </div>
	  ...
	  <c:forEach items="${list}" var="SpecificationVal" varStatus="j">
	  	<c:if test="${SpecificationVal.goodSpecificationId==goodSpecification.id}">
	  			...
	  			<c:if test="${goodSpecification.showType == '0'}">
		  			<li><a name="SpecificationVal${i.index }" id="" href="javascript:void(0)" onclick="upstyle(this,'${i.index }','${SpecificationVal.value }','${SpecificationVal.id }')">${SpecificationVal.value }</a></li>
		  		</c:if>
		  		<c:if test="${goodSpecification.showType == '1'}">
		  			<li>
			  			<a name="SpecificationVal${i.index }" id="" href="javascript:void(0)" onclick="upstyle(this,'${i.index }','${SpecificationVal.value }','${SpecificationVal.id }')"  title="${SpecificationVal.value }">
  ...
  <h2><span><img src="${ctx}/Images/images/img3.jpg" /></span><samp><a href="javascript:addGoodToFavorite('${good.id }')">收藏该资料</a></samp></h2>
  </div>
  </div><!-- 1.此款资料可以使用<a href="javascript:void(0)">分期付款</a>进行支付&nbsp;&nbsp;&nbsp;2.此资料支持<a href="javascript:void(0)">上门安装</a>服务 -->
            ...
            <c:forEach items="${buyList1}" var="good" begin="0" end="4">
	            <div class="seenProduct02_text">
	              <div class="seenProduct02_text01">
	                <div class="seenProduct02_text01_con1"><a href="${ctx }/cpxq.do?good.id=${good.goodId }"><img src="${ctx}${good.goodPic }" /></a></div>
	                
	              </div>
	               <div class="seenProduct02_text02"><a href="${ctx }/cpxq.do?good.id=${good.goodId }">${good.goodName }<samp>${good.introBrief }</samp><samp></samp></a><span>¥${good.goodPrice }</span></div>
	            </div>
            </c:forEach>
            <!--[if !IE]>产品循环结束<![endif]-->
          </div>
...
<div class="Menubox">
<ul>
<li id="one1" onclick="secBoard('one',1,6),hiddenDiv()" class="hover">资料详情</li>
<li id="one2" onclick="secBoard('one',2,6),hiddenDiv()" >规格参数</li>
<li id="one3" onclick="secBoard('one',3,6),hiddenDiv()">包装清单</li>
<li id="one4" onclick="secBoard('one',4,6),goodEvaluation(0,1)">资料评价</li>
...
<span>咨询分类：</span>
<ul>
<li><h1><input name="type4" type="radio" value="资料咨询" checked="checked"/></h1><h2>资料咨询</h2></li>
...
<div class="cartproductfluctuate" id="cart_container">
   ...
   <div id="cartItem">
     <c:forEach items="${shoppingCar.goodsInCarList }" var="good">
      <div id="cartItem_" class="shoppingCart1_text">
        <div class="shoppingCart1_text01"><a href="${ctx }/cpxq.do?good.wareId=${good.id }"><img src="${ctx}${good.pic }" border="0" title="${good.name }" /></a></div>
        <div class="shoppingCart1_text02">
          <h1>¥${good.price }</h1>
          <h2><img src="${ctx}/Images/images/img23.jpg" /></h2>
          <h3>${good.count }</h3>
          <h4>件</h4>
        </div>
        <div class="shoppingCart1_text03"><a class="delCartItem" name="11335" href="javascript:deleteGood('${good.id }');"><img src="${ctx}/Images/images/img21.jpg" border="0" /></a></div>
      </div>
     </c:forEach> 
    </div>
    <div class="shoppingCart1_text1">
      <h1><a href="${ctx }/ShoppingCar.do">更多</a></h1>
      <h2><a href="javascript:jiesuan();">去结算</a></h2>
    </div>
  </div>
</div>
	<div class="cartfluctuate" id="show_cart">
		<div class="shoppingCart" id="shoppingCarSize"><h1>已添加<span>(<c:if test="${shoppingCarSize==null}">0</c:if>${shoppingCarSize })</span>件</h1></div>
	</div>
