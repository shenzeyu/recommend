...
<link href="${ctx}/styles/front/style/pingjia2.css" rel="stylesheet" type="text/css" />
...
<li id="bor"><a href="${ctx }/pingjiaTwo.do" id="visited" >待评价商品<span>(<samp>${commentOrder}</samp>)</span></a></li>
</ul>
          </div>
        </div>
		 <!--[if !IE]>交易中心<![endif]-->
...
<table width="962" border="0" cellspacing="0" cellpadding="0" align="center">
	<c:forEach items="${saleOrderList}" var="order">
	  <tr>
	    ...
	    <td width="110" align="center" valign="middle"><div class="ys3"><fmt:formatDate value="${order.createTime }" pattern="yyyy-MM-dd"/></div></td>
	    <td width="103" align="center" valign="middle">
	    	<c:if test="${order.deliveryState=='0' && iscancel == '0'}">等待付款</c:if>
			...
			<table>
		 		<c:forEach items="${order.goodlist}" var="good">
		 			<tr style="height: 66px;">
			 			<td style="border: 0" class="ys2"><span class="cotrolys"><a href="${ctx }/pingjia.html?iid=${good.iid }&uid=${uid }&goodId=${good.id}">评价</a></span></td>
		 			</tr>
		 		</c:forEach>
		 	</table>
