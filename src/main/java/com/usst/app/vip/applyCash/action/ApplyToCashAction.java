package com.usst.app.vip.applyCash.action;
...
public class ApplyToCashAction extends BaseAction {
	...
	public String listJosn() {
		...
		List<ApplyToCash> resultList = null;
	...
	public void save() {
		...
		try {
			...
			if ("s".equals(this.applyToCash.getState())) {
				try {
					this.customerService.updatePayables(this.applyToCash.getCustomerId(),
							Double.valueOf(0.0D - this.applyToCash.getToCashAmount().doubleValue()));
				} catch (Exception e) {
