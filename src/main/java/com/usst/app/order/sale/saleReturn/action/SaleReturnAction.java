package com.usst.app.order.sale.saleReturn.action;
		...
		if ((this.wareIdArr != null) && (this.wareIdArr.length != 0)) {
			for (int i = 0; i < this.wareIdArr.length; i++) {
				...
				try {
					...
					int number = Integer.valueOf(numberStr.toString().substring(0, numberStr.toString().indexOf(".")))
							.intValue();
	...
	public void updateReturnSate() throws Exception {
		try {
			...
			if (StringUtils.isNotBlank(id)) {
				...
				this.saleReturn.setReturnState("1");
				this.saleReturnService.update(this.saleReturn);
				responseFlag(true);
			}
