package com.usst.app.order.sale.saleOrder.service;
...
public class SaleOrderService extends BaseService<SaleOrder> {
	...
	public void createSaleOrder(SaleOrder saleOrder, String[] wareIdArr, Double[] orderNumberArr, Double[] goodPriceArr,
			Double[] moneyArr, Double[] priceDiscountArr) {
	}

	public void updataDeliveryState(String orderId, String deliveryState) throws Exception {
	...
	public void updataPaymentState(String orderId, String paymentState) throws Exception {
		if (StringUtils.isBlank(orderId)) {
			throw new Exception("orderId is null");
		}
		Map<String, String> param = new HashMap();
	...
	public void updataDeliveryStateTime(String orderId) throws Exception {
		if (StringUtils.isBlank(orderId)) {
			throw new Exception("orderId is null");
		}
	...
	public void updataPaymentStateTime(String orderId) throws Exception {
		if (StringUtils.isBlank(orderId)) {
			throw new Exception("orderId is null");
		}
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	public List<SaleOrder> getSaleOrderByMonthAgo(SaleOrder saleOrder) throws Exception {
		if (saleOrder == null) {
			throw new Exception("saleOrder is null!");
		}
		List<SaleOrder> list = this.publicDAO.select("SaleOrder.SaleOrder_month", saleOrder);
		return list;
	}

	public List<SaleOrder> getSaleOrderByOrderId(String orderId) throws Exception {
		if (orderId == null) {
			throw new Exception("orderId is null!");
		}
		Map<String, String> param = new HashMap();
		param.put("orderId", orderId);
		List<SaleOrder> list = this.publicDAO.select("SaleOrder.SaleOrder", param);
		return list;
	}

	public List<SaleOrder> getSumSaleOrderList(SaleOrder saleOrder) {
		List<SaleOrder> list = this.publicDAO.select("SaleOrder.SaleOrder", saleOrder);
		return list;
	}

	public List<SaleOrder> getSaleOrderByMonthToyear(SaleOrder saleOrder) throws Exception {
		if (saleOrder == null) {
			throw new Exception("saleOrder is null!");
		}
		List<SaleOrder> list = this.publicDAO.select("SaleOrder.SaleOrder_monthtoyear", saleOrder);
		return list;
	}

	public SaleOrder getOrderByZfbTradeNo(String zfbTradeNo) throws Exception {
		SaleOrder o = null;
	...
	public List<SaleWare> getSaleWareByIdlist(String saleorderId) throws Exception {
		List<SaleWare> resultList = null;
	...
	public ByteArrayOutputStream createOrderPdf(SaleOrder saleOrder) throws Exception {
		...
		try {
			...
			BaseFont bfChinese = BaseFont.createFont("c:\\windows\\fonts\\simkai.ttf", "Identity-H", false);
			Font fontChinese = new Font(bfChinese, 12.0F, 0);

			Phrase footPhrase = new Phrase("技术支持:酷趣科技(http://www.kuquo.com)\n", fontChinese);
			...
			String picPath = Static.APACHE_CONTEXT_PATH + "/Images/index/logo.gif";
			...
			Paragraph title = new Paragraph(new Phrase("上理在线学院-订单信息", fontChinese));
			...
			deliverymoeyvalue.addElement(new Paragraph(saleOrder.getPriceDiscount().toString()));
			...
			totelInfoCell.addElement(
					new Paragraph("￥订单总额:" + totelmoney + "+配送费用:" + delivercost + "  共计:" + money, fontChinese));
