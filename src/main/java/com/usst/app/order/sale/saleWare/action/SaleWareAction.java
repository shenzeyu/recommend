package com.usst.app.order.sale.saleWare.action;
				...
				for (int i = 0; i < resultList.size(); i++) {
					...
					if (ware != null) {
						...
						if (good != null) {
							...
							((SaleWare) resultList.get(i)).setGoodStcok(Double
									.valueOf(good.getPurchaseNum().intValue() + good.getInitialNum().doubleValue()));
							if ((good.getTaxPrice() != null)
									&& (((SaleWare) resultList.get(i)).getOrderNumber() != null)) {
								((SaleWare) resultList.get(i)).setTaxMoney(
										Double.valueOf(((SaleWare) resultList.get(i)).getOrderNumber().doubleValue()
												* good.getTaxPrice().doubleValue()));
							}
						}
					}
				}
