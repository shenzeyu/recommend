package com.usst.app.order.cashBasis.collection.action;
...
import com.usst.app.order.cashBasis.collection.model.Collection;
import com.usst.app.order.cashBasis.collection.service.CollectionService;
...
public class CollectionAction extends BaseAction {
	...
	public String listJosn() {
		...
		List<Collection> resultList = null;
		...
		try {
			...
			for (int i = 0; i < resultList.size(); i++) {
				...
				if ((this.collection.getMoneyAccount() != null) || (this.collection.getMoneyCollect() != null)) {
					this.collection.setMoneyUncollect(Double.valueOf(this.collection.getMoneyAccount().doubleValue()
							- this.collection.getMoneyCollect().doubleValue()));
				}
			}
	...
	public void save() {
		...
		if ((this.bankAccountIdArr != null) && (this.bankAccountIdArr.length != 0)) {
			for (int i = 0; i < this.bankAccountIdArr.length; i++) {
				...
				if ("s".equals(this.collection.getState())) {
					try {
						this.bankAccountService.updateMoney(this.bankAccountIdArr[i],
								Double.valueOf(0.0D + this.moneyArr[i]));
					} catch (Exception e) {
						responseFlag(false);
		...
		if ("s".equals(this.collection.getState())) {
			...
			if (supplier != null) {
				try {
					this.supplierService.updatePayables(customerId,
							Double.valueOf(0.0D - this.collection.getMoneyCollect().doubleValue()));
				} catch (Exception e) {
					responseFlag(false);
				...
				if (customer != null) {
					try {
						this.customerService.updatePayables(customerId,
								Double.valueOf(0.0D - this.collection.getMoneyCollect().doubleValue()));
					} catch (Exception e) {
						responseFlag(false);
