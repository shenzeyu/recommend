...
import com.usst.app.order.price.priceAdjust.model.PriceAdjust;
import com.usst.app.order.price.priceAdjust.model.PriceAdjustWare;
import com.usst.app.order.price.priceAdjust.service.PriceAdjustService;
...
public class PriceAdjustAction extends BaseAction {
	...
	public String listJson() {
		...
		List<PriceAdjust> resultList = null;
		int totalRows = 0;
		try {
				...
				if ("1".equals(this.priceAdjust.getAdjustState())) {
					try {
						if ("2".equals(this.priceAdjust.getType())) {
							...
							Double priceChange = Double.valueOf(
									this.priceAdjustArr[i].doubleValue() - this.priceCurrentArr[i].doubleValue());

							Ware ware = (Ware) this.wareService.getModel(this.wareIdArr[i]);
							List<Ware> wareList = this.wareService.getWareByGoodId(ware.getGoodId());
							int size = 0;
							if (wareList != null) {
								size = wareList.size();
							}
