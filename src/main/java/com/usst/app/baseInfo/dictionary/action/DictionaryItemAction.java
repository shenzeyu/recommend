package com.usst.app.baseInfo.dictionary.action;
...
import com.usst.app.baseInfo.dictionary.service.DictionaryItemService;
import com.usst.app.baseInfo.dictionary.service.DictionaryService;
...
public class DictionaryItemAction extends BaseAction {
	...
	public String edit() {
		...
		try {
			if (StringUtils.isNotBlank(this.dictionaryItem.getId())) {
				this.dictionaryItem = ((DictionaryItem) this.dictionaryItemService
						.getModel(this.dictionaryItem.getId()));
			}
	...
	public void isCodeExisted() {
		...
		try {
			...
			if (this.dictionaryItem == null) {
				responseFlag(false);
			} else if ((id.equals(this.dictionaryItem.getId()))
					&& (dictionaryId.equals(this.dictionaryItem.getDictionaryId()))) {
				responseFlag(false);
			} else {
				responseFlag(true);
			}
