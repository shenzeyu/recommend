...
public final class RecommendFactory {
	 ...
	 */
	public static DataModel buildDataModelFromMysql(){
		MysqlDataSource dataSource = new MysqlDataSource();
        dataSource.setServerName(ConfigUtil.getHost());
	...
	public static RecommenderBuilder itemKNNRecommender(
			final ItemSimilarity is, final Optimizer op, final int n)
			throws TasteException {
		return new RecommenderBuilder() {
			public Recommender buildRecommender(DataModel dataModel)
					throws TasteException {
	...
	public static RecommenderBuilder treeClusterRecommender(
			final ClusterSimilarity cs, final int n) throws TasteException {
		return new RecommenderBuilder() {
			public Recommender buildRecommender(DataModel dataModel)
					throws TasteException {
	...
	public static List<String> pushItems(long uid,
			List<RecommendedItem> recommendations, boolean skip) {
		List<String> recommendList = new ArrayList<String>();
		if (!skip || recommendations.size() > 0) {
			for (RecommendedItem recommendation : recommendations) {
				recommendList.add(Long.toString(recommendation.getItemID()));
			}
		}
		return recommendList;
	}
	public static void showResult(DataModel dataModel,
			RecommenderBuilder recommenderBuilder, int recommenderNum,
			boolean skip) throws TasteException {
		LongPrimitiveIterator iter = dataModel.getUserIDs();
		while (iter.hasNext()) {
			long uid = iter.nextLong();
			List<RecommendedItem> list = recommenderBuilder.buildRecommender(
					dataModel).recommend(uid, recommenderNum);
