package com.usst.app.good.groupgood.action;
	...
	public void save() {
		...
		try {
			...
			if (StringUtils.isNotBlank(this.groupgood.getId())) {
				...
				Double moneyIn = Double.valueOf(
						this.groupgood.getInitialNum().intValue() * this.groupgood.getPurchasePrice().doubleValue());
			...
			if ((this.wareCodeArr != null) && (this.wareCodeArr.length != 0)) {
				for (int i = 0; i < this.wareCodeArr.length; i++) {
					...
					if (stop) {
						...
						Double moneyIn = Double.valueOf(this.warehouseIntoWare.getIntoNum()
				...
				if (ware == null) {
					...
					ware.setGroupgoodId(this.groupgood.getId());
					if (this.groupgood.getInitialNum() != null) {
						ware.setStock(Integer.valueOf(this.groupgood.getInitialNum().intValue()));
					}
				...
				for (int i = 1; i < 10; i++) {
					...
					for (int j = 0; j < columns; j++) {
						if ((sheet.getCell(j, i).getContents().length() == 0)
								|| (this.groupgoodService.getGoodByCode(sheet.getCell(0, i).getContents()) != null)) {
							break;
						}
						hasText = true;
						if (hasText) {
