package com.usst.app.good.goodType.service;
...
public class GoodTypeService extends BaseService<GoodType> {
	...
	public List<GoodType> pageList1(Object objParam) {
		List<GoodType> result = null;
	...
	public void getChildren(List<GoodType> typeList, String typeId) {
		if (StringUtils.isNotBlank(typeId)) {
			...
			List<GoodType> list = super.select(map);
	...
	public GoodType getSecondLevelType(String id) throws Exception {
	...
	public GoodType getByCode(String code) throws Exception {
		...
		if ((listGoodType != null) && (!listGoodType.isEmpty())) {
			for (GoodType goodType : listGoodType) {
				if (goodType != null) {
					...
					List<GoodType> children = super.select(objParam);
	...
	public GoodType getLevelType(String parentId) throws Exception {
	...
	public List<GoodType> getparentIdGoodType(String parentId) throws Exception {
		if (StringUtils.isBlank(parentId)) {
			throw new Exception("the parentId  is null!");
		}
		Map<String, String> map = new HashMap();
		map.put("parentId", parentId);
		List<GoodType> list = this.publicDAO.select("GoodType.GoodType", map);

		return list;
	}

	public GoodType getLevelTypeNo(String id) throws Exception {
		if (StringUtils.isBlank(id)) {
	...
	public List<GoodType> getGoodTypeByParentId(String parentId) throws Exception {
		if (StringUtils.isBlank(parentId)) {
			throw new Exception("the parentId  is null!");
		}
		Map<String, String> map = new HashMap();
	...
	public List<GoodType> getGoodTypeListAd() throws Exception {
		return this.publicDAO.select("GoodType.GoodType_adver", null);
	}

	public List<GoodType> selectDelAndPays(GoodType goodType) throws Exception {
		if (goodType == null) {
