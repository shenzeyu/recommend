package com.usst.app.good.goodKind.action;
...
public class GoodKingValAction extends BaseAction {
	...
	public String listJson() {
		...
		List<GoodKingVal> resultList = null;
		int totalRows = 0;
		try {
	...
	public void getKingVal() {
		...
		try {
			List<GoodKingVal> list = this.goodKingValService.getKingValByKingId(this.goodKingVal.getGoodKingId());
			String html = "";
			if (list != null) {
				for (int i = 0; i < list.size(); i++) {
					html = html
							+ "&nbsp;&nbsp;<span class='KingVal' style='border:1px solid #8DB2E3;font-size: 18px;'>&nbsp;"
							+ ((GoodKingVal) list.get(i)).getValue() + "&nbsp;"
							+ "<input type='hidden' class='KingValId' value='" + ((GoodKingVal) list.get(i)).getId()
							+ "'/></span>";
				}
			}
			responseFlag(html);
