package com.usst.app.good.goodAttr.action;
...
public class GoodAttrAction extends BaseAction {
	...
	public void getAttrList() {
		try {
			...
			List<GoodAttr> resultList = this.goodAttrService.select(this.goodAttr);
			...
			for (int i = 0; i < resultList.size(); i++) {
				...
				returnText = returnText + "<tr><td  class='gridtitle' width=\"20%\">" + this.goodAttr.getName()
						+ ":</td>";
				returnText = returnText + "<td class='gridbody'><input type=\"hidden\" value='" + this.goodAttr.getId()
						+ "' name=\"goodAttrIdArr\"/>" + "<input type=\"hidden\" value='" + this.goodAttr.getName()
						+ "' name=\"goodAttrNameArr\"/>" + "<input type=\"hidden\" value='"
						+ this.goodAttr.getInputType() + "' name=\"inputTypeArr\"/>";
				if ("0".equals(this.goodAttr.getInputType())) {
					...
					for (int j = 0; j < this.kindvalList.size(); j++) {
						...
						returnText = returnText + "'>" + ((GoodKingVal) this.kindvalList.get(j)).getValue()
								+ "</option>";
					}
				} else if ("1".equals(this.goodAttr.getInputType())) {
					returnText = returnText + "<input type='text' name='attrValueArr' id='" + this.goodAttr.getId()
							+ "' readonly='readonly'/>";
					...
					for (int j = 0; j < selectValueArr.length; j++) {
						returnText = returnText + "<option value='" + selectValueArr[j] + "'>" + selectValueArr[j]
								+ "</option>";
					}
				}
