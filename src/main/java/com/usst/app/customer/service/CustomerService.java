...
public class CustomerService extends BaseService<Customer> {
	...
	public Customer getCustomerByEmail(String code) throws Exception {
		Customer customer = null;
		if (StringUtils.isBlank(code)) {
			throw new Exception("customer email is empty,can not query customer information");
		}
		Map<String, String> map = new HashMap();
		map.put("email", code);
		Object obj = this.publicDAO.selectOne("Customer.Customer", map);
	...
	public Customer getCustomerByMobile(String mobile) throws Exception {
		...
		if (StringUtils.isBlank(mobile)) {
			throw new Exception("customer mobile is empty,can mobile query customer information");
		}
		Map<String, String> map = new HashMap();
	...
	public Boolean isSystemCustomer(String code, String pwd) throws Exception {
		...
		map.put("code", code);
		map.put("pwd", pwd);
		List<Customer> list = super.select(map);
	...
	public Boolean isSystemCustomerByEmail(String email, String pwd) throws Exception {
		Boolean isExists = Boolean.valueOf(true);
	...
	public void updateAccount(String id, String pwd, String email) throws Exception {
		if (StringUtils.isBlank(id)) {
			throw new Exception("primary key is null,can not update Customer account information");
		}
		Map<String, String> map = new HashMap();
		map.put("id", id);
		map.put("pwd", pwd);
	...
	public void updateState(String id, String state) throws Exception {
		if (StringUtils.isBlank(id)) {
			throw new Exception("primary key is null,can not update Customer Status");
		}
		Map<String, String> map = new HashMap();
		map.put("id", id);
		map.put("state", state);
	...
	public void updatePayables(String customerId, Double changeMoney) throws Exception {
		if (StringUtils.isBlank(customerId)) {
			throw new Exception("primary key is null,can not update Customer payables");
		}
		Map<String, String> para = new HashMap();
		para.put("id", customerId);

		Customer customer = (Customer) this.publicDAO.selectOne("Customer.Customer", para);
		if (customer != null) {
	...
	public void updateCredits(String id, Double credits) throws Exception {
		...
		param.put("id", id);
		Customer customer = (Customer) this.publicDAO.selectOne("Customer.Customer", param);
		if (customer != null) {
			Double newCredits = customer.getCredits();
			newCredits = Double.valueOf(newCredits.doubleValue() + credits.doubleValue());
	...
	public List<Customer> getSumCustomerList(Customer customer) {
		List<Customer> list = this.publicDAO.select("Customer.Customer", customer);
		return list;
	}
}
