package com.usst.app.warehouse.costAdjust.action;
...
import com.usst.app.warehouse.costAdjust.service.CostAdjustService;
import com.usst.app.warehouse.costAdjust.service.CostAdjustWareService;
...
public class CostAdjustAction extends BaseAction {
	...
	public String listJson() {
		...
		List<CostAdjust> resultList = null;
		int totalRows = 0;
		try {
			...
			for (int i = 0; i < this.wareIdArr.length; i++) {
				if (StringUtils.isNotBlank(this.wareIdArr[i])) {
					...
					if ("s".equals(this.costAdjust.getState())) {
						try {
							this.warehouseWareService.updateCost(this.costAdjust.getWarehouseId(), this.wareIdArr[i],
									this.costAdjustArr[i]);
						} catch (Exception e) {
							responseFlag(false);
