package com.usst.app.warehouse.warehouseMove.action;
...
import com.usst.app.warehouse.warehouseMove.model.WarehouseMove;
import com.usst.app.warehouse.warehouseMove.model.WarehouseMoveWare;
import com.usst.app.warehouse.warehouseMove.service.WarehouseMoveService;
import com.usst.app.warehouse.warehouseMove.service.WarehouseMoveWareService;
...
public class WarehouseMoveAction extends BaseAction {
	...
	public String listJson() {
		...
		List<WarehouseMove> resultList = null;
		int totalRows = 0;
		try {
			...
			for (int i = 0; i < this.fwarehousePositionIdArr.length; i++) {
				...
				if ("s".equals(this.warehouseMove.getState())) {
					...
					try {
						...
						this.warehouseService.updateCostInventory(this.warehouseMove.getFwarehouseId(),
								Double.valueOf(0.0D - this.moneyOutArr[i].doubleValue()));
					...
					try {
						...
						this.warehouseService.updateCostInventory(this.warehouseMove.getSwarehouseId(),
								this.moneyInArr[i]);
