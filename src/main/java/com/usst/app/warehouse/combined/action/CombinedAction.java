...
import com.usst.app.warehouse.combined.model.Combined;
import com.usst.app.warehouse.combined.model.CombinedWare;
import com.usst.app.warehouse.combined.service.CombinedService;
	...
	public void save() {
		...
		if ((this.wareIdArr != null) && (this.wareIdArr.length > 0)) {
			for (int i = 0; i < this.wareIdArr.length; i++) {
				...
				if ("s".equals(this.combined.getState())) {
					...
					try {
						warehouseWare = this.warehouseWareService.getWarehouseWareByWareId(this.wareIdArr[i],
								this.combined.getWarehouseId());
						...
						if ("0".equals(this.combined.getType())) {
							this.warehouseWareService.updateInventory(param,
									0 - wareCount * this.wareCountArr[i].intValue(), Double.valueOf(-1.0D));
							...
							this.warehouseWareService.updateInventory(param,
									wareCount * this.wareCountArr[i].intValue(), Double.valueOf(-1.0D));

							purchaseNum += wareCount * this.wareCountArr[i].intValue();

							totalCostInventory += averageCostInventory * wareCount * this.wareCountArr[i].intValue();
						}
						this.goodService.updateGoodInventoryInfo(good.getId(), Double.valueOf(totalCostInventory),
								Integer.valueOf(purchaseNum));
					} catch (Exception e) {
						responseFlag(false);
		...
		if ("s".equals(this.combined.getState())) {
			...
			try {
				...
				warehouseWare = this.warehouseWareService.getWarehouseWareByWareId(this.combined.getWareId(),
						this.combined.getWarehouseId());
