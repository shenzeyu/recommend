package com.usst.app.warehouse.warehouseInto.action;
...
import com.usst.app.warehouse.warehouseInto.service.WarehouseIntoService;
...
public class WarehouseIntoAction extends BaseAction {
	...
	public String listJson() {
		...
		List<WarehouseInto> resultList = null;
		int totalRows = 0;
		try {
				...
				if (StringUtils.isNotBlank(this.wareIdArr[i])) {
					...
					if ("s".equals(this.warehouseInto.getState())) {
						...
						if (!"0".equals(this.warehouseInto.getType())) {
							try {
								this.warehouseWareService.updateInventory(param, this.intoNumArr[i].intValue(),
										Double.valueOf(-1.0D));
							} catch (Exception e) {
								logger.error("error occur when updateInventory", e);
							}
							try {
								this.warehouseService.updateCostInventory(this.warehouseInto.getWarehouseId(),
										this.moneyInArr[i]);
							} catch (Exception e) {
							...
							totalCostInventory = Double
									.valueOf(totalCostInventory.doubleValue() + this.moneyInArr[i].doubleValue());
							try {
								this.goodService.updateGoodInventoryInfo(good.getId(), totalCostInventory,
