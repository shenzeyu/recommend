...
public class AdverAction extends BaseAction {
	...
	public String listPromotionActivity() {
		...
		try {
			this.promotionActivityGoodList = this.promotionActivityGoodService
					.getPromotionActivityGoodByPromoteId(this.promotionActivity.getId());
			if (this.promotionActivityGoodList.size() > 0) {
				Good good = new Good();
				for (int i = 0; i < this.promotionActivityGoodList.size(); i++) {
					good = (Good) this.goodService
							.getModel(((PromotionActivityGood) this.promotionActivityGoodList.get(i)).getGoodId());
			...
			this.promotionActivityPicList = this.fileUploadService
					.getFileUploadByPromoteId(this.promotionActivity.getId());
	...
	public String listChannel() {
		...
		try {
			...
			List<Good> goodSaleSortList = this.goodService.getGoodsByType(this.good.getGoodTypeId());
	...
	public String listPreferential() {
		...
		try {
			...
			this.promotionActivityGoodTypeList = this.promotionActivityGoodService.pageList(pageInfo,
					this.promotionActivityGood, true);
			...
			if (this.promotionActivityGoodTypeList.size() > 0) {
				...
				for (int i = 0; i < this.promotionActivityGoodTypeList.size(); i++) {
					good = (Good) this.goodService
							.getModel(((PromotionActivityGood) this.promotionActivityGoodTypeList.get(i)).getGoodId());
