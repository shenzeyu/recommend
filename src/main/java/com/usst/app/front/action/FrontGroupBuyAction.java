...
public class FrontGroupBuyAction extends BaseAction {
	...
	public String getOneGroupgood() {
		...
		if (StringUtils.isNotBlank(this.groupgood.getId())) {
			this.groupgood = ((Groupgood) this.groupgoodService.select("Groupgood.Groupgood_SY", this.groupgood)
					.get(0));
		}
		Area areas = new Area();
		areas.setLevel(3);
		...
		this.warehousecount = ((Integer) this.groupgoodService.selectToT("Groupgood.Groupgood_warehousecount_query",
				this.groupgood));

		GoodSpecificationVal goodSpecificationVal = new GoodSpecificationVal();
		goodSpecificationVal.setGoodId(this.groupgood.getId());
		this.list = this.goodSpecificationValService.select("GoodSpecificationVal.GoodSpecificationVal_groupgood_query",
	...
	public void addGroupGoodToCart() {
		...
		try {
			...
			if (this.ware != null) {
				...
				this.goodInCar
						.setTotalMoney(Double.valueOf(this.good.getPriceGroupBuy().doubleValue() * count.intValue()));
