package com.usst.code.ibatis.impl;

import com.ibatis.sqlmap.client.SqlMapClient;
import com.usst.code.bean.Base;
import com.usst.code.ibatis.AbstractDao;
import com.usst.code.ibatis.BaseSqlMapDaoSupport;
import com.usst.code.util.GUIDHelper;
...
public class PublicDAO extends BaseSqlMapDaoSupport implements AbstractDao {
	...
	public void flushDataCache(String sqlId) {
		getSqlMapClientTemplate().getSqlMapClient().flushDataCache(sqlId + "_Cache");
	}

	public long getSeqId(String seqName) {
		return ((Long) getSqlMapClientTemplate().queryForObject("getSequence", seqName)).longValue();
	}

	public Timestamp getSysDate() {
		return (Timestamp) getSqlMapClientTemplate().queryForObject("getSysDate");
	}

	public List selectBySqlMapId(String sqlId, Object objParam, int skip, int max) {
		if (max == 0) {
			return getSqlMapClientTemplate().queryForList(sqlId, objParam);
		}
		return getSqlMapClientTemplate().queryForList(sqlId, objParam, skip, max);
	}
	...
	public List dynSqlQry(String sqlText) {
		List list = getSqlMapClientTemplate().queryForList("dynInfoSql", sqlText);
		return list;
	}

	public String dynSingleInfoQry(String sqlText) {
		String strValue = null;
		List lst = getSqlMapClientTemplate().queryForList("dynSingleRowSql", sqlText);
		if (lst.size() > 0) {
	...
	public String dynMultiInfoQry(String sqlText) {
		String strValue = null;
		List lst = getSqlMapClientTemplate().queryForList("dynMultiRowSql", sqlText);
